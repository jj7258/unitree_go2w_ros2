...
def generate_launch_description():
    ...
    declare_sim_cmd = DeclareLaunchArgument(
        'sim',
        default_value='False',
        description='Launch simulation mock'
    )
    ...
    driver_cmd = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([os.path.join(
            get_package_share_directory('go2w_driver'),
            'launch/'), 'go2w_driver.launch.py'])
    )
    ...
    mock_cmd = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([os.path.join(
            get_package_share_directory('go2w_mock'),
            'launch/'), 'mock.launch.py']),
        condition=IfCondition(PythonExpression([sim]))
    )
